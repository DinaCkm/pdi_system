...
// Converter para array de CPFs únicos
const cpfsArray = [...new Set(cpfsLista.split(/\s+/).filter(cpf => cpf.trim()))];
...
async function main() {
  ...
  for (let i = 0; i < Math.min(10, rows.length); i++) {
    console.log(`"${rows[i].cpf}" (${rows[i].cpf.length} chars) -> ${rows[i].name}`);
  }
  
  // Criar mapa de CPFs do banco (sem formatação, apenas números)
  ...
  console.log(`\n=== RESUMO ===`);
  console.log(`Total de CPFs na lista: ${cpfsArray.length}`);
  console.log(`Total de CPFs no banco: ${cpfsBanco.size}`);
  
  // Verificar quais CPFs da lista existem no banco - comparando apenas números
  const existem = [];
  const naoExistem = [];
  
  for (const cpfOriginal of cpfsArray) {
    // Normalizar: remover não-dígitos e adicionar zeros à esquerda para ter 11 dígitos
    ...
    for (const [cpfBanco, nome] of cpfsBanco) {
      const cpfBancoPadded = cpfBanco.padStart(11, '0');
      
      // Comparar com padding
      if (cpfPadded === cpfBancoPadded) {
        existem.push({ cpfOriginal, cpfPadded, cpfBanco, nome });
        encontrado = true;
        break;
      }
    }
    ...
    if (!encontrado) {
      naoExistem.push({ cpfOriginal, cpfPadded });
    }
  }
  ...
  if (existem.length > 0) {
    ...
    for (const { cpfOriginal, cpfPadded, cpfBanco, nome } of existem) {
      console.log(`Lista: ${cpfOriginal} -> Padded: ${cpfPadded} -> Banco: ${cpfBanco} -> ${nome}`);
    }
  }
  
  if (naoExistem.length > 0) {
    console.log(`\n=== CPFs NÃO ENCONTRADOS (${naoExistem.length}) ===`);
    for (const { cpfOriginal, cpfPadded } of naoExistem.slice(0, 20)) {
      console.log(`Lista: ${cpfOriginal} -> Padded: ${cpfPadded}`);
    }
    if (naoExistem.length > 20) {
      console.log(`... e mais ${naoExistem.length - 20} CPFs`);
    }
  }
